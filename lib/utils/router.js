...
class Router extends KoaRouter {
   ...
   * POST   | /posts/:id/:actionName | actionName     | app.controller.posts[actionName]
   *
  ...
  resources(...args) {
    ...
    for (const key of keys) {
      ...
      if (REST_MAP[key]) {
        ...
        const path = `${prefix}/${REST_MAP.show.suffix}/${key}`;
        this.register(path, [ 'POST' ], middleware.concat(action), { name: key });
      }
    }
